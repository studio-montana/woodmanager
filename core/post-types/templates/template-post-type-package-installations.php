...
<input type="hidden" name="<?php echo WOODMANAGER_PACKAGE_INSTALLATIONS_NONCE_ACTION; ?>" value="<?php echo wp_create_nonce(WOODMANAGER_PACKAGE_INSTALLATIONS_NONCE_ACTION);?>" />
...
if (!$can_update){
	?>
	<p class="">
		<?php _e("You can not view package installations, please edit original item of this translation.", WOODMANAGER_PLUGIN_TEXT_DOMAIN); ?>
	</p>
	<p class="">
	...
	<table>
		...
		if ($has_package){
			...
			<?php
			$installations = BD_Package_Installation::get_package_installations("id_package = ".$bd_package->id);
			if (!empty($installations)){
				foreach ($installations as $installation){
					$date = $installation->date_modif;
					$date_s = '';
					if (!empty($date)){
				...
				?><tr><td colspan="2"><span><?php _e("no installation for this package", WOODMANAGER_PLUGIN_TEXT_DOMAIN); ?></span></td></tr><?php
			}
		}else{
